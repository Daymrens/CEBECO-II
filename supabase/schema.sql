...
-- ---------------------------------------------------------------------
-- Audit log (every admin mutation is recorded here)
--   action:     create | update | cancel | delete
--   target_type: outage | user | subscriber
--   details:    free-form JSON string of what changed (before/after, ids)
-- ---------------------------------------------------------------------
create table if not exists public.audit_logs (
  id            uuid primary key default gen_random_uuid(),
  actor_user_id uuid references public.users (id) on delete set null,
  action        text not null check (action in ('create', 'update', 'cancel', 'delete')),
  target_type   text not null check (target_type in ('outage', 'user', 'subscriber')),
  target_id     text,
  details       jsonb,
  created_at    timestamptz not null default now()
);
