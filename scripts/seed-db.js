...
async function run() {
  ...
  await pool.query(
    `INSERT INTO users (name, email, password_hash, is_admin) VALUES ($1, $2, $3, $4) ON CONFLICT (email) DO UPDATE SET password_hash = $3`,
    ['Admin', 'admin@cebeco.example', hash, true]
  );
  ...
  for (const o of outages) {
    await pool.query(
      ...
      [o.title, o.municipality, o.barangays, o.type, o.status, o.date, o.start_time, o.end_time, o.reason, o.source || 'manual', o.source_url || null]
    );
  }
  console.log('Real outages seeded:', outages.length);

  // 3. Verify
  const r1 = await pool.query('SELECT count(*) FROM outages');
  const r2 = await pool.query('SELECT count(*) FROM users');
  console.log('Total outages:', r1.rows[0].count);
  console.log('Total users:', r2.rows[0].count);
